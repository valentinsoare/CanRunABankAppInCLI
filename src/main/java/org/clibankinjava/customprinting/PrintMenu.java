...
public class PrintMenu {
    ...
    public static void of(IMenu menu) throws InterruptedException {
        ...
        if (options.size() <= 1) {
            PrintError.toConsole(StructuralErrors.NO_MENU_OPTION_AVAILABLE, menu.getEmptySpacesFromLeftEdgeScreen(),  200,
                    false, true);
            System.exit(0);
        }
        ...
        for (int i = 0; i < menu.getNumberOfEntriesInTheMenu(); i++) {
            System.out.printf("%s[ %d ] %s%n", " ".repeat(menu.getEmptySpacesFromLeftEdgeScreen()), (i+1), options.get(i));
        }
